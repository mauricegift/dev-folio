...
const Sidebar = memo(({ 
  ...
  return (
    <>
      ...
      <aside 
        className={`fixed top-0 left-0 z-[70] w-[75vw] h-full bg-white dark:bg-gray-900 border-r-2 border-gray-200 dark:border-gray-800 transition-transform duration-300 lg:hidden flex flex-col will-change-transform ${
            ...
            {navLinks.map(link => {
              ...
              return (
                <button 
                  ...
                  className={`w-full text-left px-6 py-4 rounded-2xl font-bold flex items-center justify-between transition-all duration-200 ${
                    active 
                      ? 'bg-indigo-600 text-white shadow-lg' 
                      : 'text-gray-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20'
                  }`}
                >
...
const ScrollToTopButton = memo(({ 
  ...
  return (
    <button 
      onClick={onClick}
      className={`fixed bottom-24 right-4 sm:right-6 z-[90] w-12 h-12 sm:w-14 sm:h-14 bg-white dark:bg-gray-900 rounded-full flex items-center justify-center border-2 border-gray-100 dark:border-gray-800 shadow-xl transition-all duration-300 hover:scale-110 active:scale-95 group ${
    ...
    >
      <svg className="absolute w-full h-full -rotate-90 pointer-events-none" viewBox="0 0 50 50">
        ...
        <circle 
          ...
          strokeLinecap="round" 
          className="text-indigo-600 dark:text-indigo-500 transition-all duration-150" 
        />
      </svg>
      <div className="relative flex flex-col items-center justify-center">
        <svg className="w-4 h-4 sm:w-5 sm:h-5 text-indigo-600 dark:text-indigo-500 group-hover:-translate-y-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 15l7-7 7 7" />
        </svg>
        <span className="text-[7px] sm:text-[8px] font-black text-indigo-600 dark:text-indigo-500 uppercase mt-0.5">
...
const Breadcrumbs = memo(({ 
  ...
  return (
    <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 md:px-10 mb-2 flex-shrink-0">
      <nav className="flex items-center space-x-2 text-[11px] text-gray-500 dark:text-gray-400 bg-gray-50/50 dark:bg-gray-900/50 w-fit px-4 py-1.5 rounded-full border-2 border-gray-200 dark:border-gray-800 backdrop-blur-sm shadow-sm overflow-x-auto whitespace-nowrap scrollbar-hide">
        {breadcrumbs.map((crumb, idx) => (
          <React.Fragment key={crumb.id}>
            {idx > 0 && (
              <svg className="w-3.5 h-3.5 opacity-50 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
              </svg>
            )}
            <button 
              onClick={() => handleNavClick(crumb.id)}
              className={`flex items-center gap-1.5 uppercase tracking-widest transition-colors flex-shrink-0 ${
                idx === breadcrumbs.length - 1 || isNavActive(crumb.id) 
                  ? 'font-black text-indigo-600 dark:text-indigo-400' 
                  : 'font-bold hover:text-indigo-600'
              }`}
            >
...
const Layout: React.FC<LayoutProps> = ({ children, activePage, setActivePage }) => {
  ...
  // Initial load and scroll effects - optimized with requestAnimationFrame
  useEffect(() => {
  ...
  // Sidebar body scroll lock - optimized
  useEffect(() => {
    if (isSidebarOpen) {
  ...
  const isNavActive = useCallback((id: string) => {
  ...
  const breadcrumbs = useMemo(() => {
    ...
    if (activePage.startsWith('blog-detail-')) {
      ...
      crumbs.push({ 
        name: blog ? (blog.title.length > 15 ? blog.title.substring(0, 12) + "..." : blog.title) : 'Detail', 
        id: activePage, 
        icon: NavIcons.blogs 
      });
      ...
      crumbs.push({ 
        name: project ? project.title : 'Detail', 
        id: activePage, 
  ...
  return (
    <div className="min-h-screen flex flex-col bg-transparent text-gray-900 dark:text-gray-100 relative">
      <style>{`
        @keyframes heartbeat {
          0% { transform: scale(1); }
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 px-2 sm:px-4 pt-4 h-20 sm:h-24">
        <div className="max-w-7xl mx-auto h-full bg-white/70 dark:bg-gray-950/70 backdrop-blur-xl border-2 border-gray-200 dark:border-gray-800 rounded-2xl sm:rounded-[2rem] shadow-sm flex justify-between items-center px-4 sm:px-6 md:px-10">
          <div className="flex items-center">
            <button 
              onClick={() => setIsSidebarOpen(true)} 
              className="p-2 mr-2 lg:hidden text-gray-500 bg-gray-100 dark:bg-gray-800 rounded-full hover:bg-indigo-50 dark:hover:bg-indigo-900/40 transition-colors"
            ...
            >
              <img 
                src={WEB_INFO.logoUrl} 
                alt="Logo" 
                className="w-8 h-8 sm:w-10 sm:h-10 md:w-14 md:h-14 object-contain transition-transform group-hover:scale-110"
                loading="eager"
              />
            ...
            {navLinks.map(link => {
              ...
              return (
                <button 
                  key={link.id} 
                  onClick={() => handleNavClick(link.id)}
                  className={`text-[11px] font-black uppercase tracking-[0.2em] relative py-2 flex items-center gap-2 transition-colors ${
                    active 
                ...
                >
                  {link.icon} {link.name}
                  {active && (
                    <span className="absolute bottom-0 left-0 right-0 h-[3px] bg-indigo-600 rounded-full"></span>
                  )}
                </button>
              );
            })}
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            <button 
              onClick={toggleTheme} 
              className="p-2 sm:p-2.5 rounded-lg sm:rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/40 transition-colors shadow-sm"
            ...
            <a 
              href={`https://${DEV_INFO.github}`} 
              target="_blank" 
              rel="noopener noreferrer" 
              className="hidden sm:flex bg-gray-900 dark:bg-white text-white dark:text-black w-10 h-10 sm:w-12 sm:h-12 rounded-full items-center justify-center hover:scale-110 shadow-lg transition-all will-change-transform"
              aria-label="GitHub"
            >
      ...
      <main className="flex-grow pt-20 sm:pt-28 md:pt-32 z-10 flex flex-col">
        <Breadcrumbs 
      ...
      <footer className="bg-white/40 dark:bg-gray-950/40 backdrop-blur-xl border-t-2 border-gray-300 dark:border-gray-800 py-12 md:py-16 z-10 rounded-t-2xl md:rounded-t-[3rem] mt-8 md:mt-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10">
          <div className="mb-8 md:mb-12 text-center md:text-left">
            <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-400 mb-6">
              Transmission Grid
            </h4>
            <div className="flex flex-wrap justify-center md:justify-start gap-3">
              {socialLinks.map(s => (
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="p-2.5 md:p-3 bg-white/50 dark:bg-gray-900/50 border-2 border-gray-100 dark:border-gray-800 rounded-xl md:rounded-2xl text-gray-500 hover:bg-indigo-600 hover:text-white hover:border-indigo-600 transition-all hover:scale-110 shadow-sm will-change-transform"
                  title={s.id}
                  aria-label={s.id}
                >
                  {s.icon}
          ...
          <div className="pt-6 md:pt-8 border-t-2 border-gray-300 dark:border-gray-800 flex flex-col md:flex-row justify-between items-center gap-4 md:gap-6">
            <span className="text-gray-500 dark:text-gray-400 text-[10px] font-black uppercase tracking-[0.3em] text-center md:text-left">
              © 2024 — {new Date().getFullYear()} {displayName} 
              <span className="animate-heartbeat inline-block ml-1">💛</span>
            </span>
            <div className="flex gap-4 md:gap-8">
              <button 
                onClick={() => handleNavClick('privacy')} 
                className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 hover:text-indigo-500 transition-colors"
              >
                Privacy
              </button>
              <button 
